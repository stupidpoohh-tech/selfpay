...
상단의 `PC 화면으로 보기` 를 누르면 데스크톱 배치를 그대로 줄여서 봅니다. 뷰포트 폭을
1440 으로 바꿔 브라우저가 축소하는 방식이라 화면을 다시 만들지 않습니다. 세로로 들면
작아서 확대가 필요하고, 가로로 들면 그대로 읽힙니다. 선택은 브라우저에 남아서 다시
열어도 유지되고, `모바일 화면으로` 로 되돌립니다. 좁은 화면에서만 버튼이 나옵니다.
