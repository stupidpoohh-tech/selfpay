...
| **복합기** | 대기·연결 · 복사(복사 시작 · 작업 완료 · 결제 요청) · 스캔 · 팩스 |
...
- 큰 화면 안의 버튼 자리도 누를 수 있습니다. 좌표는 비교 화면과 같은 `assets/js/shot.js`
  히트박스를 그대로 씁니다. 다만 **이 흐름 안의 단계로 이어지는 자리만 남깁니다**(`다음 단계: 홈`).
  전체 흐름에서 누른 것이 모바일·복합기 화면으로 건너뛰지 않습니다.
  이어지는 자리가 없는 곳을 누르면 아무 일도 일어나지 않습니다.
- 원본 크기 보기는 모바일·복합기 개별 비교 화면에서 그대로 씁니다. 흐름의 큰 화면은
  이미 크게 보여 주고 있으므로 눌러도 확대하지 않습니다.
...
복사 묶음은 `복사 시작` → `복사`(설정 확인·결제 대기) → `작업 완료` → `결제 요청` 순서입니다.
`복사 시작` 은 AS-IS 설정 화면 하나가 TO-BE에서 두 상태로 나뉜 자리라 AS-IS 그림을 함께 씁니다.

변경점이 여섯 개를 넘는 화면은 번호·라벨을 여백에 다 세울 수가 없어, 모두 켠 상태에서는
번호만 표시 영역 모서리에 붙이고 고른 항목만 유형 라벨을 내놓습니다(`.cmp--dense`).
그 아래 화면들의 표시 방식은 그대로입니다.
...
| 복합기 연결 (복사·스캔·팩스) | 홈 → 복사·스캔·팩스 → QR 스캔 / 시리얼번호 입력 → 연결됨 |
| 설정 | 홈 → 설정 → 요금 안내 · 결제 내역 · 환불 안내 · 문제 해결 |
| 이력 | 홈 → 이력 → 영수증 보기 · 환불 요청 · 결제하기 |
| 알림 | 홈 → 알림 → 영수증 보기 · 작업 상세 |

복합기 연결은 카메라나 QR 인식을 하지 않습니다. 복사·스캔·팩스 모두 연결된 상태만
화면 위에 보여 줍니다.
...
- 목적지 화면이 없어 눌러도 넘어가지 않는 버튼: 결제 화면의 결제하기(출력 화면 없음),
  복사 화면의 다음 단계로, 미리보기, 문의하기, 결제 내역의 각 항목,
  요금 안내의 용지·옵션 행
