...
전체 흐름의 세 항목은 화면 한 장이 아니라 구조 변화라서 전용 배치를 씁니다.
왼쪽에서 단계를 고르고, 가운데에서 그 단계를 크게 보고, 오른쪽에서 관련 개선 사항을 읽습니다.

```
[단계 목록]        [고른 단계 화면]        [개선 사항]
 AS-IS 1 2 3 4      AS-IS 큰 화면           01 관찰한 문제
   ↓ 2단계 축소      TO-BE 큰 화면           02 변경한 내용
 TO-BE 1 2          ‹ 이전 단계  다음 단계 ›  03 …
```

- 단계를 누르면 그 단계가 파랗게 켜지고, 그 화면이 크게 바뀌고, 관련 개선 사항만 진해집니다.
  고르지 않은 단계도 지우지 않고 옅게 남겨 지금 어디인지 알 수 있게 둡니다.
- 관련 개선 사항은 `screens.js` 의 단계에 적힌 `ref` (notes 번호)로 정합니다.
- 흐름 안의 `이전 단계 / 다음 단계` 는 그 흐름의 단계를 넘기고, 위쪽 이전·다음은 흐름 항목을
  넘깁니다. 좌우 방향키는 단계를 넘기다가 끝에서 흐름 항목으로 넘어갑니다.
- 고른 단계는 주소에 `step=a3`(AS-IS 3번째) · `step=b1`(TO-BE 1번째)로 남습니다.
- 화면을 누르면 기존 원본 보기가 그대로 열립니다.
- `모바일 ↔ 복합기` 는 AS-IS가 없는 신규 흐름이라 한 줄짜리 배치로 나옵니다.
  빈 AS-IS 자리를 두지 않습니다. 단계마다 `모바일`(파랑) · `복합기`(초록) 표시가 붙습니다.
- 폰에서는 단계 목록이 가로로 넘어가고, 화면과 개선 사항이 위아래로 놓입니다.
