...
[영역]  전체 흐름 | 모바일 | 복합기                              현재: 홈  ‹ 1 / 19 ›
[선택]  홈 · 로그인 · 인쇄 · 복사·스캔·팩스 · 설정 · 알림 · 이력
[본문]  AS-IS 스크린샷 | TO-BE 스크린샷 | 개선 사항 노트
```

맨 위에서 영역을 셋 중에 고릅니다. 영역을 바꾸면 그 아래 화면 목록이 통째로 바뀌고,
이전·다음 이동도 고른 영역 안에서만 돕니다.

| 영역 | 안에 있는 것 |
| --- | --- |
| **전체 흐름** | 첫 진입, 인쇄 설정·결제 통합, 모바일 ↔ 복합기 연결·결제 |
| **모바일** | 홈 · 로그인 · 인쇄 · 복사·스캔·팩스 · 설정 · 알림 · 이력 |
| **복합기** | 대기·연결 · 복사 · 스캔 · 팩스 |

모바일 영역에서 고를 수 있는 것은 대표 화면 일곱 개입니다. 딸린 화면(인쇄 아래 파일확인·
인쇄옵션·금액확인, 설정 아래 요금 안내·결제 내역·환불 안내·문제 해결 등)을 보고 있어도
목록에서는 대표 화면이 선택된 것으로 보이고 옆에 지금 화면 이름이 붙습니다. 영역과 묶음은
`assets/js/screens.js` 의 `surfaces` 에 있습니다.

### 전체 흐름

전체 흐름의 세 항목은 화면 한 장이 아니라 구조 변화입니다. **흐름 전체는 위에 늘 펼쳐 두고**,
단계를 눌러도 흐름은 그대로 있고 아래 큰 화면과 강조 자리만 바뀝니다. 넘겨 가며 흐름을 숨기지
않습니다.

```
AS-IS  [1 로그인] → [2 계정 선택] → [3 홈] → [4 작업 선택]      4단계
                     ↓ 2단계 축소
TO-BE  [1 홈] → [2 작업 선택]                                  2단계

[ AS-IS 큰 화면 ]     [ TO-BE 큰 화면 ]     [ 개선 사항 · 핵심 변화 4단계 → 2단계 ]
```

- 단계는 클릭 전에도 전부 보입니다. 누른 단계만 파랗게 켜지고 나머지는 옅게 남습니다.
- AS-IS와 TO-BE는 각각 고릅니다. 한쪽을 눌러도 반대쪽 화면은 그대로 있습니다.
  단계 수가 다른 흐름이라 1:1 로 맞물려 넘어가지 않습니다.
- **화면이 따로 없는 행동 단계**(계정 선택, 작업 선택)는 빈 자리를 만들지 않습니다.
  그 행동이 일어나는 화면을 그대로 쓰고 그 자리를 짚어 줍니다. 좌표는 새로 만들지 않고
  `screens.js` 의 단계에 적은 `hl: { screen, change, side }` 로 **그 화면의 변경점 좌표를
  그대로 가져옵니다.** 짚을 자리가 분명하지 않으면 `hl` 을 두지 않고 화면만 보여 줍니다.
- 화면을 누르면 기존 원본 보기가 그대로 열립니다.
- 개선 사항은 흐름 전체 설명이라 늘 같은 자리에 같은 내용으로 있습니다. 맨 위에 `핵심 변화`
  한 줄(양쪽 `summary` 에서 그대로 만듭니다)을 두고, 고른 단계와 관련된 항목에만 옅은 표시를
  더합니다. 단계를 넘길 때마다 목록이 바뀌는 것처럼 보이지 않게 둡니다.
- 흐름 안에는 이전·다음 버튼을 두지 않습니다. 단계를 직접 누르는 것으로 충분합니다.
  위쪽 이전·다음(`1 / 3`)과 좌우 방향키는 흐름 항목 사이를 옮깁니다.
- 주소는 `?surface=flow&screen=entry` 그대로입니다. 고른 단계는 주소에 남기지 않고,
  다시 열면 각 흐름의 첫 단계에서 시작합니다.
- `모바일 ↔ 복합기` 는 AS-IS가 없는 신규 흐름이라 한 줄 배치로 나옵니다.
  빈 AS-IS 자리를 두지 않습니다. 단계마다 `모바일`(파랑) · `복합기`(초록) 표시가 붙습니다.
- 폰에서는 흐름 줄이 가로로 넘어가고, 큰 화면과 개선 사항이 위아래로 놓입니다.
- 이 영역에서는 프로토타입 보기를 내놓지 않습니다.

모바일 영역의 `비회원` 도 흐름 항목이지만 개별 화면 비교와 함께 보는 자리라
기존 단계 목록 표시를 그대로 씁니다.

복합기 화면은 `shots/device-*.png`(AS-IS)와 `proposal/shots/device-*.png`(TO-BE)를 씁니다.
가로로 긴 기기 화면이라 화면 데이터에 `wide: true` 를 두고, 비교에서 두 장이 가로 폭을
나눠 갖고 개선 노트는 아래로 내려갑니다. 변경점과 기대 효과는 다른 화면과 같은 방식으로
`screens.js` 에 있습니다.

- 스크린샷은 원본 비율을 유지한 채 뷰포트 높이에 맞춰 축소됩니다. 두 장이 같은 높이로 보이고,
  화면을 보기 위한 세로 스크롤이 생기지 않습니다.
- 스크린샷을 누르면 원본 크기로 봅니다. 다시 누르거나 Esc로 닫습니다.
- 좌우 방향키로도 화면을 넘길 수 있습니다. 원본 보기가 열려 있으면 뒤 화면은 움직이지
  않고, Esc 로 닫습니다.
- 비교 화면 안의 버튼 자리를 누르면 그 화면 비교로 옮겨 갑니다. 좌표는 프로토타입에서 쓰는
  `assets/js/shot.js` 히트박스를 그대로 다시 씁니다.
- 주소에 영역과 현재 화면이 남습니다. 예: `?surface=mobile&screen=print`
- 예전 주소(`?screen=print`)도 그대로 열립니다. 화면 이름으로 어느 영역인지 알아냅니다.
...
인쇄·복합기 연결·설정 하위 이동, 영역 전환과 주소 호환, 키보드, AS-IS / TO-BE 가 서로를
대신하지 않는지, 화면 맵이 그려지는지, 모바일 폭에서 가로로 넘치지 않는지를 봅니다. Playwright 가
...
- 복합기 화면의 프로토타입. 기기 쪽은 눌러 볼 페이지가 없습니다.
- `모바일 ↔ 복합기 연결·결제` 의 AS-IS 흐름. 현재 순서를 확인하기 전이라 비워 두었습니다.
