      ...
      proposal: {
        ...
        steps: [
          { label: '파일 확인 및 결제',
            ...
            items: ['선택 파일 확인', '파일 미리보기', '파일별 설정',
                    '전체 파일 기본 설정 확인·변경', '예상 결제 금액 확인', '결제 진입'],
      ...
      changes: [
        {
          ...
          targets: {
            ...
            proposal: [{ x: 20.5, y: 16.2, w: 58.5, h: 4.8 }]
          }
        },
        {
          ...
          targets: {
            ...
            proposal: [{ x: 4.7, y: 21.8, w: 90.1, h: 67.9 }]
          }
        },
        {
          ...
          targets: {
            ...
            proposal: [{ x: 4.7, y: 21.8, w: 90.1, h: 42.4 }]
          }
        },
        {
          id: 'per-file', type: 'add', shortLabel: '기능 추가',
          title: '파일별 설정 기능 추가',
          description: 'AS-IS에서는 인쇄 옵션이 선택한 파일 전체에 한꺼번에 적용되어 ' +
            '파일마다 다른 설정으로 인쇄할 수 없었습니다. TO-BE에서는 파일마다 `설정` 버튼을 두고, ' +
            '따로 설정한 파일에는 `개별 설정` 표시를 붙였습니다. 아래 설정은 `전체 파일 기본 설정`이 ' +
            '되어 개별 설정하지 않은 파일에만 적용되며, ' +
            '`개별 설정 파일 전체 설정으로 초기화`로 되돌릴 수 있습니다.',
          targets: {
            proposal: [{ x: 81.5, y: 29.1, w: 11.5, h: 31.0 },
                       { x: 46.5, y: 34.4, w: 9.8, h: 2.6 },
                       { x: 41.5, y: 48.8, w: 9.8, h: 2.6 },
                       { x: 56.0, y: 78.6, w: 37.0, h: 2.2 }]
          }
        }
      ],
      effects: [
        '결제 전에 확인할 것을 한 화면에서 볼 수 있습니다',
        '파일마다 다른 설정으로 인쇄할 수 있습니다',
        '어떤 파일을 따로 설정했는지 목록에서 구분할 수 있습니다'
      ]
    },
    {
      ...
      changes: [
        {
          ...
          targets: {
            ...
            proposal: [{ x: 4.7, y: 65.5, w: 90.1, h: 16.4 }]
          }
        },
        {
          ...
          targets: {
            proposal: [{ x: 4.7, y: 83.9, w: 90.1, h: 5.8 }]
          }
        },
        {
          id: 'steps', type: 'restructure', shortLabel: '단계 축소',
      ...
      changes: [
        ...
        },
        {
          ...
          targets: {
            ...
            proposal: [{ x: 5.5, y: 90.0, w: 89.0, h: 3.1 }]
          }
        },
        {
          id: 'steps', type: 'restructure', shortLabel: '단계 축소',
...
(function (R) {
      ...
      changes: [
        {
          ...
          targets: {
            ...
            proposal: [{ x: 53.5, y: 24.0, w: 22.2, h: 55.0 }]
          }
        },
        {
          ...
          description: 'AS-IS에서는 QR을 스캔했을 때 어떤 기기와 연결되는지 화면에서 바로 확인하기 ' +
            '어려웠습니다. TO-BE에서는 QR 안내 문구에 ' +
            '`QR을 스캔하면 휴대폰이 4번 복합기와 연결됩니다`라고 적어, ' +
            '스캔하기 전에 연결될 기기를 확인할 수 있도록 했습니다.',
          targets: {
            proposal: [{ x: 8.3, y: 48.0, w: 38.5, h: 4.6 }]
          }
        },
        {
          ...
          targets: {
            ...
            proposal: [{ x: 8.0, y: 23.6, w: 43.2, h: 56.0 }]
          }
        }
      ],
      effects: [
        '한 화면에서 스캔해야 할 QR을 구분하는 부담을 줄일 수 있습니다',
        '스캔하기 전에 어느 복합기와 연결되는지 확인할 수 있습니다',
      ...
      changes: [
        {
          ...
          targets: {
            ...
            proposal: [{ x: 32.0, y: 18.5, w: 63.0, h: 62.5 }]
          }
        },
        {
          ...
          description: 'AS-IS에서는 복사 요금 정보와 설정 영역의 관계가 약했습니다. ' +
            ...
            '구성했습니다.',
          targets: {
            proposal: [{ x: 7.5, y: 63.8, w: 23.0, h: 16.4 }]
          }
        },
        {
          id: 'reset', type: 'add', shortLabel: '기능 추가',
          title: '세팅 초기화 추가',
          description: 'AS-IS에서는 골라 둔 설정을 한 번에 되돌리는 방법이 없어 컬러·출력 방식·부수를 ' +
            '하나씩 다시 눌러야 했습니다. TO-BE에서는 설정 영역 오른쪽 위에 `세팅 초기화`를 두어 ' +
            '바꾼 설정을 기본값으로 한 번에 되돌릴 수 있도록 했습니다.',
          targets: {
            proposal: [{ x: 80.0, y: 20.4, w: 13.2, h: 6.2 }]
          }
        },
        {
          ...
          description: 'AS-IS에서는 복합기 화면에서 `결제하기`와 `복사 시작`이 함께 제공되어 ' +
            '결제와 실행의 관계가 분명하지 않았습니다. TO-BE에서는 복합기에서 `결제하기`를 없애고, ' +
            '`설정을 확인한 뒤 휴대폰에서 결제를 진행해 주세요. 결제가 완료되면 자동으로 복사가 ' +
            '시작됩니다`라는 안내를 두어 결제는 휴대폰이 맡는다는 것을 적었습니다.',
          targets: {
            current:  [{ x: 69.3, y: 87.8, w: 26.8, h: 8.4 }],
            proposal: [{ x: 31.5, y: 84.3, w: 37.5, h: 7.8 }]
          }
        }
      ],
      effects: [
        '현재 복사 설정과 예상 비용을 함께 확인할 수 있습니다',
        '바꾼 설정을 한 번에 기본값으로 되돌릴 수 있습니다',
  ...
  var PROTO = {
    'device-home': {
      ...
      hits: [
        { l: 19.6, t: 64.2, w: 10.0, h: 15.4, to: 'device-copy-start', name: '복사', next: true },
        { l: 30.4, t: 64.2, w: 10.0, h: 15.4, to: 'device-scan',       name: '스캔' },
        { l: 41.2, t: 64.2, w: 10.0, h: 15.4, to: 'device-fax',        name: '팩스' }
      ]
    },
    'device-copy-start': {
      hits: [
    ...
    'device-copy': {
      /* 세팅 초기화는 화면을 옮기지 않고 설정만 되돌리는 자리라 뺐다 */
      hits: [
        { l: 6.6,  t: 10.3, w: 10.4, h: 5.2, to: 'device-home',      name: '홈으로' },
        { l: 73.5, t: 84.2, w: 20.4, h: 8.0, to: 'device-copy-done', name: '복사 시작', next: true }
      ]
    },
    'device-copy-done': {
      hits: [
