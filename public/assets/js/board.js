...
(function (w) {
  ...
  var ICONS = {
    ...
    pay:'<rect x="3" y="5.4" width="18" height="13.2" rx="2.4"/><path d="M3 10h18"/><path d="m9.4 15.2 1.7 1.7 3.5-3.5"/>',
  ...
  /* ── 전체 흐름 전용 보기 ───────────────────── */
  /* 전체 흐름은 화면 한 장이 아니라 구조 변화다. 흐름 전체는 늘 위에 펼쳐 두고,
   * 단계를 누르면 그 쪽 큰 화면과 강조 영역만 바뀐다. 흐름을 넘겨 가며 숨기지 않는다. */
  ...
  function hlBoxes(st) {
    if (!st || !st.hl) return [];
    var s = byId(st.hl.screen);
    if (!s) return [];
    var c = (s.changes || []).filter(function (x) { return x.id === st.hl.change; })[0];
    return c ? SPAnno.boxesOf(c, st.hl.side) : [];
  }

  /* ── 흐름 막대: 모든 단계가 늘 보인다 ── */
  function nodeHtml(s, side, st, i) {
    var on = i === stepAt(s, side);
    return '<button type="button" class="fnode' + (on ? ' is-on' : '') +
      '" data-fside="' + side + '" data-fstep="' + i + '" aria-pressed="' + on + '">' +
      '<span class="fnode__n">' + (i + 1) + '</span>' +
      '<span class="fnode__t">' + whereHtml(st, 'fxwhere') + st.label + '</span>' +
    '</button>';
  }

  function rowHtml(s, side, solo) {
    var d = s[side];
    var nodes = d.steps.map(function (st, i) {
      return (i ? '<span class="fsep" aria-hidden="true">→</span>' : '') + nodeHtml(s, side, st, i);
    }).join('');
    return '<div class="fxrow">' +
      (solo ? '' : '<span class="tag tag--' + (side === 'current' ? 'as">AS-IS' : 'to">TO-BE') + '</span>') +
      '<div class="fxrow__nodes">' + nodes + '</div>' +
      (d.summary ? '<em class="fxrow__sum">' + d.summary + '</em>' : '') +
    '</div>';
  }

  /* 두 줄 사이에 무엇이 달라졌는지. 문구는 데이터에서 그대로 나온다 */
  function midHtml(s) {
    var a = s.current.steps.length, b = s.proposal.steps.length;
    var word = b === 1 && a > 1 ? '하나로 통합' : (b < a ? a - b + '단계 축소' : '');
    return '<div class="fxmid">' +
      '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" ' +
      'stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' +
      '<path d="M12 5v13"/><path d="m6 13 6 6 6-6"/></svg>' +
      (word ? '<span>' + word + '</span>' : '') + '</div>';
  }

  function barHtml(s) {
    var sides = flowSides(s);
    var solo = sides.length === 1;
    return '<div class="fxbar">' +
      rowHtml(s, sides[0], solo) +
      (solo ? '' : midHtml(s) + rowHtml(s, sides[1], solo)) +
    '</div>';
  }

  /* ── 큰 화면: 고른 단계의 화면과 강조 영역만 바뀐다 ── */
  function fpaneHtml(s, side) {
    ...
    /* 강조는 바깥을 한 장으로 덮고 짚을 자리만 뚫는다.
     * 자리마다 덮개를 따로 두면 서로를 가린다. */
    var boxes = hlBoxes(st);
    var hl = boxes.length
      ? '<svg class="fxveil" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">' +
          '<path fill-rule="evenodd" d="M0 0H100V100H0Z' +
            boxes.map(function (b) {
              return 'M' + b.x + ' ' + b.y + 'H' + (b.x + b.w) + 'V' + (b.y + b.h) + 'H' + b.x + 'Z';
            }).join('') + '"/></svg>' +
        boxes.map(function (b) {
          return '<span class="fxhl" style="left:' + b.x + '%;top:' + b.y + '%;width:' + b.w +
            '%;height:' + b.h + '%"></span>';
        }).join('')
      : '';
    ...
    return '<section class="fxpane" data-fpane="' + side + '">' +
      '<div class="fxpane__head">' +
        (solo ? '' : '<span class="tag tag--' + (side === 'current' ? 'as">AS-IS' : 'to">TO-BE') + '</span>') +
        '<span class="fxpane__n">' + (i + 1) + '</span>' +
  ...
  /* 핵심 변화 한 줄. 양쪽 summary 에서 그대로 만든다 */
  function keyChange(s) {
    var sides = flowSides(s);
    if (sides.length < 2) return (s[sides[0]] || {}).summary || '';
    var a = s.current.summary, b = s.proposal.summary;
    return a && b ? a + ' → ' + b : (b || a || '');
  }
  ...
  function flowNotesHtml(s) {
    ...
    return '<aside class="notes notes--flow">' +
      ...
      (key ? '<div class="fxkey"><span>핵심 변화</span><b>' + key + '</b></div>' : '') +
  ...
  /* 그림 상자를 원본 비율로 맞춘다. 강조 영역의 % 가 그림에 그대로 얹히게 하려는 것이다 */
  function sizeFits() {
    el.stage.querySelectorAll('.fxshotfit').forEach(function (fit) {
      var im = fit.querySelector('.fxshot');
      if (!im) return;
      ratio(im.getAttribute('src'), function (r) { fit.style.aspectRatio = String(r); });
    });
  }
  ...
  /* 흐름 막대는 그대로 두고 누른 쪽 화면만 바꾼다 */
  function redrawFlow(side) {
    var s = screen();
    var root = document.getElementById('fx');
    if (!root) return draw();

    root.querySelectorAll('.fnode[data-fside="' + side + '"]').forEach(function (b) {
      var on = +b.getAttribute('data-fstep') === stepAt(s, side);
      b.classList.toggle('is-on', on);
      b.setAttribute('aria-pressed', String(on));
    });

    var pane = root.querySelector('[data-fpane="' + side + '"]');
    if (pane) pane.outerHTML = fpaneHtml(s, side);
  ...
  /* 개선 사항은 흐름 전체 설명이라 그대로 두고, 관련 항목에만 옅은 표시만 둔다 */
  function syncFlowNotes() {
    var s = screen();
    var refs = {};
    flowSides(s).forEach(function (sd) {
      ((s[sd].steps[stepAt(s, sd)] || {}).ref || []).forEach(function (n) { refs[n] = 1; });
    });
    el.stage.querySelectorAll('.note[data-note]').forEach(function (n) {
      n.classList.toggle('is-hit', !!refs[+n.getAttribute('data-note')]);
    });
  }
